<template>
  <div class="row">
                    ...
                    <td>
                      ...
                      <div class="pt-3">
                        ...
                        <a
                          ...
                          @click.prevent="deleteAttribute(item)"
                          class="text-primary pr-2"
                          >Delete</a
                        >
                        <a
                          href="javascript:void(0)"
                          @click.prevent="showNewChildAttributeModal(item)"
                        >
                          Add new {{ item.title }}
                        </a>
                      </div>
    ...
    <modal
      v-if="showNewChildModalStatus"
      @close="showNewChildModalStatus = false"
      size="lg"
    >
          ...
          <div class="row">
            <div class="col">
              ...
              <input
                type="text"
                class="form-control"
                v-model="childAttributeForm.name"
              />
            </div>
            <div
              class="col"
              v-if="activeAddNewModalAttribute.title.toLowerCase() == 'color'"
            >
              <label>Color code</label>
              <input
                type="text"
                class="form-control"
                v-model="childAttributeForm.info"
              />
            </div>
          </div>
        ...
        <div class="mt-2">
          <table class="table table-bordered">
            ...
            <tbody>
              ...
              <tr
                v-for="(child, index) in childAttributeModal.data"
                :key="index"
                v-else
              >
                <td colspan="3" class="d-flex justify-content-between">
                  <div>
                    {{ child.title }}
                    <span v-if="child.info != null">({{ child.info }})</span>
                  </div>
                  <div>
...
export default {
  ...
  methods: {
    ...
    deleteAttribute(attribute) {
      this.$swal({
        ...
        .then((res) => {
          if (res.isConfirmed) {
            axios
              .post("attributes/" + attribute.id + "/delete", {
                _method: "DELETE",
              })
              .then((res) => {
    ...
    showNewChildAttributeModal(attribute) {
      ...
      this.getChildAttribute(
        "attributes/children/" + this.activeAddNewModalAttribute.id + "/all"
      );
    ...
    async getChildAttribute(actionLink) {
      ...
      await axios
        ...
        .catch((error) => {
          this.$swal(
            "Some went wrong while fetching the children attributes",
            "",
            "error"
          );
          console.log(error);
    ...
    addNewChildAttribute() {
      ...
      axios
        .post("attributes/children/add-new", {
    ...
    deleteChild(child) {
      axios
        .post(
          "attributes/children/" + child.parent_id + "/" + child.id + "/delete",
          {
            _method: "DELETE",
          }
        )
